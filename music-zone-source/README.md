# Music Zone plaintext sources

This directory is the local-only source for the encrypted `/music/` page:

```text
music-zone-source/<album-slug>/cover.jpg
music-zone-source/<album-slug>/<song-slug>/<song-title>_<singing-style>_<YYYYMMDD>.m4a
music-zone-source/<album-slug>/<song-slug>/lyrics.lrc
music-zone-source/<album-slug>/<song-slug>/lyrics.txt
music-zone-source/<album-slug>/<song-slug>/lyrics.meta.json
```
...
- Set `MUSIC_ZONE_PASSWORD` in the ignored `.env` file, then run `pnpm music:encrypt` after adding or changing any cover, lyric, or recording.
- The encryption command publishes only AES-GCM ciphertext in `public/music-zone/encrypted` and refreshes the encrypted catalog payload. Never move originals back into `public`.
...
- The page accepts either `lyrics.lrc` or `lyrics.txt`, preferring LRC when both exist. Lyrics and metadata are embedded inside the encrypted catalog payload.
- After encryption, rebuild the site so album progress and version lists update.
- Only publish lyrics, covers, and recordings you are authorized to use.

The plaintext contents below this README are ignored by Git. Back them up separately: a fresh clone or worktree will not contain your covers, lyrics, or recordings. Generated ciphertext is safe to commit, but the password must remain only in `.env` or the build environment.
